...
class ExecutionChecker {
    ...
    public static void checkExecution(Execution execution,
            ...
            if (RDF.TYPE.equals(statement.getPredicate())) {
                ...
                if (statement.getPredicate().stringValue().equals("http://etl.linkedpipes.com/ontology/pipeline")) {
                    output.add(valueFactory.createStatement(
                            ...
                            statement.getObject(),
                            graph));
                } else if (statement.getPredicate().stringValue().equals("http://etl.linkedpipes.com/ontology/status")) {
                    output.add(valueFactory.createStatement(
                            statement.getSubject(),
                            statement.getPredicate(),
                            statement.getObject(),
        ...
        output.add(valueFactory.createStatement(
                ...
                valueFactory.createLiteral(componentsCount),
                graph));
    ...
    public static void loadPipeline(Execution execution)
            throws IOException, MissingFile {
        //
        File pipelineFile = new File(execution.getDirectory(), "pipeline.jsonld");
        if (!pipelineFile.exists()) {
            pipelineFile = null;
            // Look in the definition directory.
            File directory = new File(execution.getDirectory(), "definition");
            if (!directory.exists()) {
                throw new MissingFile("For execution: " + execution.getIri());
            }
            //
        ...
        output.add(valueFactory.createStatement(
                ...
                valueFactory.createIRI("http://linkedpipes.com/ontology/Pipeline"),
                graph));

        output.add(valueFactory.createStatement(
                valueFactory.createIRI(execution.getIri()),
                valueFactory.createIRI("http://etl.linkedpipes.com/ontology/pipeline"),
                pipelineResource,
                graph));
