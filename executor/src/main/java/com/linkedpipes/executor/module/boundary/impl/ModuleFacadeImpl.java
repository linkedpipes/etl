package com.linkedpipes.executor.module.boundary.impl;
...
public class ModuleFacadeImpl implements ModuleFacade, ApplicationListener<ApplicationEvent> {

    private static final Logger LOG = LoggerFactory.getLogger(ModuleFacadeImpl.class);
     ...
     */
    private static final String QUERY_COMPONENT_INFO = ""
            + "SELECT ?path WHERE {\n"
            + "  <%s> <" + LINKEDPIPES.HAS_JAR_URI + "> ?path .\n"
            + "}";
    ...
    protected void start() {
        ...
        config.put(Constants.FRAMEWORK_SYSTEMPACKAGES_EXTRA, EXPORT_PACKAGE_LIST);
        config.put(Constants.FRAMEWORK_STORAGE, configuraiton.getOsgiStorageDirectory());
        ...
        // Load libraries.
        scanDirectory(configuraiton.getOsgiLibDirectory(), (file) -> {
            if (file.getPath().endsWith(".jar")) {
                LOG.info("Installing: {}", file);
                try {
                    libraries.add(context.installBundle(file.toURI().toString()));
                } catch (BundleException ex) {
                    appContext.stop();
    ...
    protected void stop() {
        ...
        if (framework != null) {
            try {
                ...
                LOG.error("Interrupted when waiting for framework to close.", ex);
            }
        }
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (event instanceof ContextStartedEvent) {
            ...
            // FelixDispatchQueue is thread used by felix, it's created as non deamon. For this reason
            // we need to manualy stop the framework and not wait on OnDestroy call.
            stop();
        }
    }
    ...
    public Component getComponent(DefinitionStorage definition, String subject) throws ModuleException {
        ...
        try {
            final List<Map<String, String>> resultList
                    = definition.executeSelect(String.format(QUERY_COMPONENT_INFO, subject));
            if (resultList.size() != 1) {
                throw new ModuleException("Invalid number query results!");
            }
            componentInfo = resultList.get(0);
        } catch (QueryException ex) {
            throw new ModuleException("Can't load information about componenet.", ex);
        }
        ...
        if (!components.containsKey(jarFileUri)) {
            ...
            try {
                ...
                throw new ModuleException("Can't load bundle from given location!", ex);
            }
            try {
                bundle.start();
            } catch (BundleException ex) {
        ...
        final BundleContext componenetContext = components.get(jarFileUri).getBundleContext();
        // Use manager to get the component representation.
        for (ComponentFactory factory : getServices(ComponentFactory.class)) {
            try {
                return factory.createComponent(definition, subject, definition.getDefinitionGraphUri(),
                        componenetContext);
            } catch (ComponentFactory.InvalidBundle ex) {
                throw new ModuleException("Invalid bundle detected!", ex);
            }
        }
        throw new ModuleException("No factory can handle given type of jar file.");
    }

    @Override
    public ManagableDataUnit getDataUnit(DefinitionStorage definition, String subject)
            throws ModuleException {
        for (DataUnitFactory factory : getServices(DataUnitFactory.class)) {
            try {
                ManagableDataUnit dataUnit = factory.create(definition, subject, definition.getDefinitionGraphUri());
                if (dataUnit != null) {
                    return dataUnit;
                }
        ...
        throw new ModuleException("No factory can instantiace required data unit.");
    }
     ...
     * @throws ModuleException
     */
    public <T> Collection<T> getServices(Class<T> clazz) throws ModuleException {
        final BundleContext context = framework.getBundleContext();
        try {
            final Collection<ServiceReference<T>> references = context.getServiceReferences(clazz, null);
            ...
            LOG.trace("Detected {} of type {}", serviceList.size(), clazz.getSimpleName());
            return serviceList;
